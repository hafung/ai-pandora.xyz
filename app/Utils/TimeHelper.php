...
class TimeHelper {
     ...
     */
    public static function toFriendly($datetime, string $lang = 'zh-CN'): string {
        ...
        if ($interval->y) {
            $count = $interval->y;
            $type = $lang == 'zh-CN' ? '年' : ' year';
        } elseif ($interval->m) {
            $count = $interval->m;
            $type = $lang == 'zh-CN' ? '月' : ' month';
        } elseif ($interval->d) {
            $count = $interval->d;
            $type = $lang == 'zh-CN' ? '天' : ' day';
        } elseif ($interval->h) {
            $count = $interval->h;
            $type = $lang == 'zh-CN' ? '小时' : ' hour';
        } elseif ($interval->i) {
            $count = $interval->i;
            $type = $lang == 'zh-CN' ? '分钟' : ' minute';
        } elseif ($interval->s) {
            $count = $interval->s;
            $type = $lang == 'zh-CN' ? '秒' : ' second';
        }

        if (empty($type)) {
            return $lang == 'zh-CN' ? '未知' : 'unknown';
        } else {
            return $count . $type . ($lang == 'zh-CN' ? '前' : (($count > 1 ? 's' : '') . ' ago'));
        }
    }
